...
class AddressesStore {
  ...
  static final state =
      ValueNotifier<StoreState<List<BuyerAddress>>>(const StoreLoading());
  static StreamSubscription<List<BuyerAddress>>? _sub;
  static int _refCount = 0;

  static List<BuyerAddress>? get current =>
      state.value is StoreLoaded<List<BuyerAddress>>
          ? (state.value as StoreLoaded<List<BuyerAddress>>).data
          : null;
  ...
  static void _subscribe() {
    ...
    _sub = BuyerRepository().watchAllAddresses().listen(
      (addresses) => state.value = StoreLoaded(addresses),
