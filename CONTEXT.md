...
**SaleStage**:
A single-value summary of the current blocking stage of a Sale, derived from its AssemblyStatus, Payment, and Shipment. Four values: `assembly` (not yet assembled), `payment` (assembled but unpaid — takes priority over delivery state), `shipment` (assembled, paid, not yet delivered), `done` (assembled, paid, and delivered). Used on SaleCard to replace the former SaleProgressPath with a single scannable chip.
...
3. **Sales list** — default view shows only **active Sales** (not yet delivered); timeline grouped Overdue → This week → Next week → Later → past months. Tune icon opens a filter/sort sheet with grouped filters (Money / Logistics / Compliance), year chips (one per year that has Sales, dynamically generated), date range picker (mutually exclusive with year chips), inline buyer search (single-select). Sort is a separate AppBar popup with its own badge. Selecting a year chip shows **all** Sales in that calendar year including delivered ones, overriding the active-only default; grouping switches to creation month. Selecting a date range does not override the default (still hides delivered). Map icon in AppBar navigates to the standalone SalesHeatMap screen. Each Sale shown as a card: buyer name + derived total price top row; first SaleItem description (single truncated line) + attention badges right — tapping the description row expands it inline to show all SaleItem descriptions (each still truncated to one line), a chevron icon signals the affordance, the full-width row has a minimum 48px tap target, tapping anywhere else on the card navigates to the detail screen; creation date left + scheduled date right; a single **SaleStage** chip at the bottom (no divider) showing the current blocking stage — **Assembly** if not yet assembled, **Payment** if assembled but unpaid (even if already delivered), **Shipment** if assembled and paid but not yet delivered, **Done** if assembled, paid, and delivered. Category chips are not shown on the card (visible in SaleDetailScreen only). Left accent bar 6–8px wide: red = overdue with blockers, amber = this week with blockers. Attention badges (tap to open detail sheet): `receipt_long` purple = NIF unfiled (also shown in orange when no NIF on file), green = NIF filed; `price_check` = assembly ready but unpaid; specific blocker icon = single urgency reason, generic ⚠️ = multiple.
