...
**BuyerAddress**:
A named shipping address belonging to a Buyer (e.g. "Home", "Work"). Fields: label, country (default: Portugal), postal code, city, street name, optional house number, and optional fraction (e.g. "2º Dto", "R/C"). House number is omitted for institutional addresses that have none (e.g. a tribunal, hospital, or named building). For Portuguese addresses, entering the postal code auto-fills city and street via GeoAPI.pt (180-day local device cache). A Buyer may have multiple BuyerAddresses; one may be marked as default.
...
**SaleStage**:
A single-value summary of the current blocking stage of a Sale, derived from its AssemblyStatus, Payment, and Shipment. Four values: `assembly` (not yet assembled), `payment` (assembled but unpaid — takes priority over delivery state), `shipment` (assembled, paid, not yet delivered), `done` (assembled, paid, and delivered).
_Avoid_: Status, phase, step, stage (without "Sale" prefix)
...
**Shipment**:
The physical delivery of Items from a Sale to a Buyer. Has a `DeliveryType` (`shipping`, `pickup`, or `handDelivery`), a status, an optional CTT tracking code, a postal code used for geographic analytics, and an optional `shippedAt` timestamp. References a BuyerAddress for `shipping` and `handDelivery`. Status flow differs by type: `shipping` goes `pending → shipped → delivered`; `pickup` and `handDelivery` go `pending → delivered` (the `shipped` state is never written for those). Hand delivery is seller-delivered within the same city — address and postal code are required (used in the SalesHeatMap), tracking code is absent. `shippedAt` is auto-captured when the status first moves to `shipped` and is editable afterwards (from the "Mark as Shipped" bottom sheet and the Sale detail Delivery section); it is only meaningful for `DeliveryType.shipping`.
...
**GeographicSalesView**:
A geographic breakdown of Sales by location, accessed from the Sales list AppBar (map icon). Default view is a **ranked list**: a Portugal section (one row per CP4 locality, sorted by the active metric) and an International section (one row per country, for foreign shipments). A sticky SegmentedButton (Revenue / Sales count) sits between the AppBar and the scrollable list — always visible regardless of scroll position, hidden when in map sub-mode. An AppBar icon button toggles to a **map sub-mode** (heat map over a flutter_map tile layer) for spatial intuition; the list is the default. Sales are grouped by 4-digit postal code prefix (e.g. 3000-550 and 3000-313 both map to "3000"). Only Sales with a postal code are included (shipped and hand-delivery); in-person Sales without a postal code are excluded. A `directions_walk` AppBar toggle lets the seller include or exclude hand deliveries; default is included.
_Formerly_: SalesHeatMap
_Avoid_: Geographic report, map view, heat map (refers to the map sub-mode only)

**ShoppingList**:
An aggregated view of all materials still needed to complete open Sales. Groups same-named ComponentItems (case-insensitive exact match) across all open SaleItems and sums their quantities into a single row — e.g. "blue bead × 5" rather than two separate rows. Filtered to Sales that are not yet assembled and not yet delivered. Rows are ordered by worst-case urgency of any contributing Sale (overdue → this week → none). Each aggregated row is expandable: tapping it reveals the per-SaleItem breakdown (buyer name · item description · quantity contribution), and each breakdown row navigates to the Sale detail screen. Notes and photo badges are shown only in the expanded per-SaleItem rows, not at the aggregated level. No upload or edit from this screen.
...
**DriveBackup**:
An off-platform safety copy of all app data stored in the seller's Google Drive under `Latitude Tracker Backup/`. Contains two sub-folders: `data/` (one versioned JSON file per year, using the same schema as Archive exports) and `photos/` (all SaleItem, ComponentItem, and Repair photos mirrored from Firebase Storage, organised into three sub-folders: `sales/{saleId}/{itemId}/{uuid}.jpg`, `components/{saleId}/{itemId}/{componentId}/{uuid}.jpg`, and `repairs/{repairId}/{uuid}.jpg`). Updated automatically once per day via a background job that runs only on WiFi. Photos are uploaded incrementally — already-present files are skipped; new photos since the last run are uploaded. Incremental checks use a Drive custom property (`isUploaded: true`) queried in bulk at backup start, so per-photo existence checks are O(1) in-memory lookups against a pre-built UUID Set. Individual photo upload failures are skipped (not retried in the same run) and reported as `BackupPartialSuccess(int failed)`; the timestamp is still saved so partial completion is visible. The JSON data is fully re-exported on every run (it is tiny). The seller can also trigger a backup manually from Settings. Both devices participate in the daily backup; the incremental design makes concurrent runs from two devices safe and idempotent. Progress is reported in two phases: "Uploading data…" (JSON phase) then "Uploading photos… N / total" (photo phase).
_Avoid_: Archive (Archive is a year-specific read-only historical export; DriveBackup is the live off-platform safety copy)

**BackupRestore**:
The process of recovering app data from a DriveBackup after data loss, scoped to one or more years chosen by the seller. Tapping Restore from Drive lists the years found in `Latitude Tracker Backup/data/` (all pre-selected by default, covering the common case of restoring everything); the seller can deselect years they don't need before confirming. Only the selected years run through two phases: (1) JSON restore — each selected year's JSON file is downloaded from Drive and imported into Firestore using the same logic as Archive import (existing documents are skipped; new ones are created); (2) photo restore — scoped automatically to the selected years, since photo UUIDs are extracted only from the JSON files actually processed in phase 1. Each photo is downloaded from Drive and re-uploaded to Firebase Storage at its original path (`users/{uid}/sales/{saleId}/items/{itemId}/photos/{uuid}.jpg`), restoring photo URLs to a working state. Because photo paths are identical to the originals, all Firestore document `photoUrls` resolve correctly after restore without any document edits.
  ...
  "sales": [
    {
      ...
      "shipment": { "type": "shipping|pickup|handDelivery", "status": "pending|shipped|delivered",
                    ...
                    "shippedAt": "<ISO-8601 string or null>" },
...
3. **Sales list** — default view shows only **active Sales** (not yet delivered); timeline grouped Overdue → This week → Next week → Later → past months. Tune icon opens a filter/sort sheet with grouped filters (Money / Logistics / Compliance), year chips (one per year that has Sales, dynamically generated), date range picker (mutually exclusive with year chips), inline buyer search (single-select). Sort is a separate AppBar popup with its own badge. Selecting a year chip shows **all** Sales in that calendar year including delivered ones, overriding the active-only default; grouping switches to creation month. Selecting a date range does not override the default (still hides delivered). Map icon in AppBar navigates to the standalone SalesHeatMap screen. Each Sale shown as a card with a **tri-indicator strip** on the left edge (replacing the former accent bar and SaleStage chip): three vertically-stacked icons, one per domain area, each color-coded by state. Assembly slot (`build` icon): `waiting_for_materials` = red, `not_started` = grey, `in_progress` = amber, `ready` = green. Payment slot (`payments_outlined` icon): unpaid = red, paid = green. Shipment slot (type-specific icon — `local_shipping` for shipping, `store` for pickup, `directions_walk` for hand delivery): pending = red, in-transit = blue (shipping only), delivered = green; pickup and hand delivery skip the blue state. Card content to the right of the strip: buyer name + derived total price top row; item descriptions row (first item truncated, expandable inline to show all, chevron signals affordance, 48px minimum tap target); dates row with up to three slots pinned left / centre / right — bought date (`shopping_bag_outlined`, always shown), shipped date (`local_shipping_outlined`, only when `shippedAt` is set on a `shipping`-type Shipment), and scheduled date (`event`, shown when `scheduledDate` is set); no bottom chip row. Attention badges (tap to open detail sheet): `receipt_long` purple = NIF unfiled (orange when no NIF on file), green = NIF filed; `price_check` = assembly ready but unpaid; specific blocker icon = single urgency reason, generic ⚠️ = multiple. Category chips are not shown on the card (visible in SaleDetailScreen only).
...
5. **Sale detail** — live stream; section order: Payment (icon), Items/Assembly (icon), Delivery (icon), Scheduled Date (below Delivery, no icon), Repairs section, Notes. SaleItems list (description, category, price per item; tap item to see its assembly status, component checklist, and photos); derived total price; payment toggle; unified NIF/AT compliance row that progresses through four states — (1) no NIF on file (with inline "Add NIF" dialog that saves to Buyer without leaving the screen), (2) NIF receipt required but AT not yet filed, (3) AT pending, (4) AT filed — shown when `requiresNif`; delivery card with status, address, CTT tracking code, and editable `shippedAt` field (shown only for `shipping` type); Scheduled Date card sits immediately below Delivery; **Repairs section** (only shown when at least one Repair is linked to this Sale — lists each Repair with description, RepairStatus, and date; each row taps to Repair detail); Notes card (inline editable); buyer name taps to Buyer detail. Each section header (Assembly, Payment, Shipment) shows the same color-coded status icon used in the SaleCard tri-indicator, so the seller can instantly see which area needs attention without scrolling. The three tri-indicator sections (Payment, Items/Assembly, Delivery) additionally **auto-collapse** when their status is already green (paid / all SaleItems ready / delivered) — computed once when the screen loads and not re-evaluated as the live stream updates, so a section never collapses out from under the seller mid-interaction. A collapsed header still shows a one-line summary (e.g. "Card · Paid", "3 items · Ready", "Shipping · Delivered") alongside the status dot, and the seller can tap any header to expand or re-collapse it manually; this manual state is not persisted — reopening the screen always re-derives collapse state from current status. Scheduled Date, Repairs, and Notes are never auto-collapsed (no derived "done" state).

5a. **Repairs list** — inner tab within the Sales/Repairs area. Default view shows only **active Repairs** (RepairStatus ≠ `returned`, or ReturnDelivery status ≠ `delivered`). No AppBar — a filter row sits at the top of the body: a search chip (expands to a text field, searches contact name and item description) and a tune icon with a badge counting active constraints. Tune sheet contains: date sort (newest/oldest toggle) and RepairStatus filter chips; selecting any chip overrides the active-only default and shows only that status. FAB creates a new Repair. Each Repair shown as a card with the same tri-indicator strip structure as SaleCard (payment → work → return delivery dots). Card content to the right of the strip: contact name + materials cost (top row, cost shown only when set); item description (primary line) with problem description snippet below it (muted, truncated to one line); received date (bottom row). No ItemCategory chip on the card (visible in Repair detail only).
...
5c. **Repair detail** — live stream; section order mirrors Sale detail: Contact, Payment (icon), Items (item description, category, problem, received date — no icon), Work/RepairStatus (icon), ReturnDelivery (icon), Photos, Linked Sale. Contact row (taps to Buyer detail if linked, "Promote to Buyer" action if free-text); Payment card; item description, category, problem description; Work done field (inline editable); materials cost; RepairStatus picker; ReturnDelivery card; photos; linked Sale row (if set, taps to Sale detail). When RepairStatus is `done` or `returned` and ReturnDelivery is not yet `delivered`, the Return card shows a quick-action button: "Mark as Sent" (shipping, pending→shipped) or "Mark as Delivered" (all other cases). Pressing "Mark as Delivered" also auto-advances RepairStatus to `returned`. The RepairCard tri-indicator strip order mirrors SaleCard: payment → work → return delivery (top to bottom).
...
- BuyerAddress house number clarified: the field is optional — institutional addresses (tribunal, hospital, named building) may have no number. When absent, it is omitted from the formatted address and from the maps URI. The form no longer requires it.
...
- Sale detail accordion collapse (#193) clarified: only the 3 tri-indicator sections (Payment, Items/Assembly, Delivery) auto-collapse when green; a Sale where all 3 are green collapses fully rather than forcing one section open, relying on the new collapsed-header summary line to keep the screen non-blank. Collapse state is derived once on screen load and frozen thereafter (live stream updates don't force a section shut mid-interaction); it is not persisted across navigations.
